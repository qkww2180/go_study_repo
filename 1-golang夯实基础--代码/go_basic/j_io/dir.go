...
func CreateFile(fileName string) {
	...
	os.Mkdir("../z_data/sys", os.ModePerm)          //创建目录并设置权限
	os.MkdirAll("../z_data/sys/a/b/c", os.ModePerm) //增强版Mkdir，沿途的目录不存在时会一并创建

	// os.Rename("../z_data/sys/a", "../z_data/sys/p")       //给文件或目录重命名
	// os.Rename("../z_data/sys/p/b/c", "../z_data/sys/p/c") //Rename还可以实现move的功能

	// os.Remove("../z_data/sys")    //删除文件或目录，目录不为空时才能删除成功
	// os.RemoveAll("../z_data/sys") //增强版Remove，所有子目录会递归删除
}
